...
public class AutoShooter {
    ...
    private static final double[][] points = {{31,51,2510}, {42.9,51,2680}, {62,43,2980}, {81.5,39,3200}, {96,40,3450}, {119,39,3710}, {130,37,3910}, {148.45,36.8,4210}};//distance, angle, rpm
...
//        return Math.sqrt(g * (Math.sqrt(Math.pow(getGoalDistance(robotPose, teamColor), 2) + Math.pow(goalHeight, 2)) + goalHeight));
    ...
    public static double normalizeDegreeError(double error) {//degrees
        error = (error + 180) % 360;
        if (error < 0)
            error += 360;
        return error - 180;
    }public static double normalizeRadianError(double error) {//degrees
        return Math.toRadians(normalizeDegreeError(Math.toDegrees(error)));
    }
